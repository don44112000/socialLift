...
|Content-Type {Required}|string|Request message type.<br>Allowed format: `multipart/form-data`,`application/json`. <br>Use `multipart/form-data` when the `upload_type` is `UPLOAD_BY_FILE`. |
```
