...
<img src="https://sf16-sg.tiktokcdn.com/obj/tiktok-open-platform-sg/ttop_doc_images/27ce7b196b1eb3e587e0dc44f81674f9" alt="image" width="2164.000000px" />
...
1. Import the `TikTokOpenAuthSDK` module and create an authorization request, as shown in the code snippet below. In the authorization request init method, set the following parameters.
...
1. Start the login by calling the `send(_:)` method on the authorization request object.
1. After the user finishes authorizing, you will receive a response callback in the closure. In the authorization response object, you can find whether the authorization request succeeded or failed.
...
Upload the `TikTokAuthResponse.code` returned in the callback and the `TikTokAuthRequest.pkce.codeVerifier` to your server-side and obtain a user access token. See [User Access Token Management](/doc/oauth-user-access-token-management) for more information. 
### Handling errors
