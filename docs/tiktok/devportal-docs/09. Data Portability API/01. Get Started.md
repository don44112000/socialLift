...
1. Go to the **Manage apps** page and select your app, or **Connect an app** if you don't have one yet.
1. On the app page, click the **Add products** button under **Products**, then add **Data Portability API**.
1. Add the **Login Kit** and **Webhooks** products to your app as well. 
1. On the **Data Portability API** product, view scopes, then click **Apply** to access the application for Data Portability API.
1. Fill out the **Data Portability API Application** form and submit. Make sure to select all necessary scopes on the form. 
  1. Wait for approval. You can typically expect to hear back within 3-4 weeks. You can check for scope approval status on the app page. 
  1. Once you have received approval, you can choose which approved scopes to add to your app on the **Data Portability API** product. 

1. Fill out the information on the app page, including under **Login Kit** and **Webhooks**.
1. Click **Submit for review**, and we will start reviewing your app.
...
1. Request data
  1. Use the [Add Data Request](https://developers.tiktok.com/doc/data-portability-api-add-data-request/) endpoint to start a data export. In your request, you should indicate the categories of data you're requesting (all_data, activity, video, profile, direct_message), which the user should have authorized permission for. If they haven't authorized permission, the response will include an error.

1. (If desired) Check the status of an ongoing request
  1. Use the [Check Status of Data Request](https://developers.tiktok.com/doc/data-portability-api-check-status-of-data-request/) endpoint to see the status of an ongoing request.

1. (If desired) Cancel an ongoing request
  1. Use the [Cancel Data Request](https://developers.tiktok.com/doc/data-portability-api-cancel-data-request/) endpoint to cancel a request. If the user has allowed you to make ongoing requests, you can make another request using the same token.

1. Once a request is ready, we'll send notice to the callback URL you provided in the app page via [webhook](https://developers.tiktok.com/doc/webhooks-events/). We aim to provide data within a few seconds, minutes, or hours, depending on the data requested. However, this is not guaranteed, as unforeseen technical issues may cause delays.
1. Once you receive the callback, you can [download the data](https://developers.tiktok.com/doc/data-portability-api-download/) with a Download request. Data will be available to download for four days after it's been prepared.
