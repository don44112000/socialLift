...
See User Info Object below for a full list of values.
...
username as the unique identifier || 
...
The url to a user's profile picture || 

"is_verified" || 

bool || 

The user's verified status. True if verified, false if not || 
...
The number of videos the user has posted || 

"likes_count" || 

int || 

The total number of likes the user has accumulated || 

"bio_url" || 

string || 

The url in user's bio when shared  |
### Example
