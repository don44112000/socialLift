...
Content type for the return data || 
...
The requested fields. Choose from the Comment Object fields.
...
The ID of the video that the comments are made to || 
...
The ID of the comment that the replies are made to || 
...
The number of comments in response. 

Default is 10, max is 100.

It is possible that the API returns less comments than the max count due to reasons such as comments deleted by users etc. || 
...
The starting index of the comments in the response.
...
A list of comment objects for a given video
...
The ID of the video or item that the comment is under || 
...
The number of likes a comment has || 

reply_count || 

int || 

The number of replies a comment has || 

create_time || 

int || 

The unix timestamp that the comment was created on || 

display_name || 

string || 

The display name of the user that left the comment |
### Example
