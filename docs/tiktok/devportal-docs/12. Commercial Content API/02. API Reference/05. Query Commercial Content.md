...
id,create_timestamp,brand_names,creator,videos || 
...
Filters that will be applied to the query. || 
...
The max number of results returned at once. The default value is 10, and the maximum value is 50. || 
...
"eyJsYXN0X3NvcnQiOlsyNTQxMTkwLCIzNDk1NzA4NjI0N" || 
...
The time range during which the commercial contents were published.

"min" needs to be after October 1st, 2022. || 
...
The country of the commercial content's author. The default value is ALL.

Supported countries: European Economic Area (EEA) countries [Supported Countries](/doc/commercial-content-api-supported-countries)

Note: United Kingdom and Switzerland are not included in this API || 
...
The commercial contents' creators.
 || 
...
The first date of the range and this needs to be after October 1st, 2022. || 
...
list<CommercialContent> || 

The list of commercial contents. ||  || 
...
If you update the`filters` in the request, please remove the `search_id` to avoid getting back unexpected results || 
...
The create date of the commercial content in format of YYYYMMDD. || 
...
The create date of the commercial content in format of Unix timestamp. || 
...
The label of this commercial content. || 
...
The brands that sponsor this commercial content. || 
...
The commercial content creator's public information. ||  || 

video || 

CommercialContentVideo || 

The commercial content video's public information. ||  |
### CommercialContentCreator
...
The commercial content creator's TikTok handler. || 
...
The commercial content video's ID. || 
...
The commercial content video's status. || 
...
The commercial content video's URL. || 

https://www.tiktok.com/@joe1234567/video/19384729204821234 || 

cover_image_url || 

string || 

The commercial content video's cover image URL. || 
