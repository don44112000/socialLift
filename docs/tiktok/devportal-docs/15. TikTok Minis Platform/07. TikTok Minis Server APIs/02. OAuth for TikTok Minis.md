...
Note: OAuth for TikTok Minis has the same structure as [User Access Token Management](/doc/oauth-user-access-token-management), with the exception of omitting `redirect_uri` and `code_verifier` in the request body parameters for fetching an access token. 
...
Your app's unique client key, obtained from your app page on the Developer Portal || 

client_secret || 

string || 

Your app's unique client secret, obtained from your app page on the Developer Portal || 
...
A fixed value that should always be set as `authorization_code` |
### Response struct
...
The expiration time of `refresh_token` in seconds || 

token_type || 

string || 

A fixed value that should be set to `Bearer`. |
...
A fixed value that should always be set as `refresh_token` || 
