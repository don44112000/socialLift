...
Tier ID of the subscription product || 
...
The order info from partner's side || 
...
The ID of your order created in your system || 
...
The name of the product user intended to purchase || 
...
The URL to the order information page in your system || 
...
Detailed info of the order info, if any || 
...
The ID of order created by TikTok. Please store this ID since you can only query the order status through this ID. || 
...
ID of user's current active subscription || 
...
list<ThirdPartyUserSubscriptionObject>
...
is_subscription_rights_valid || 

bool || 

indicate whether this right is still valid || 
...
indicate whether the renew is normal, false if renew is failed/onold || 
...
the latest trade_order_id related to this subscription || 
...
beginning time of this subscription || 
...
IAP or WEB; IAP=Google/Apple Store, WEB=PayPal || 
...
same as the requested trade_order_id || 
...
indicate the status of the trade order: SUCCESS, PENDING,REFUNDED || 
...
beginning time of the subscription related to this trade order || 
...
end time of the subscription related to this trade order || 
...
Happen time of the next deduction of the subscription related to this trade order || 
...
The list of tier_ids of the target subscription product.

For sandbox, these `tier_ids` can be used: 
1. sandbox_499_1M  $4.99/Monthly
1. sandbox_1347_3M $13.47/QUARTERLY
1. sandbox_699_1M $6.99/Monthly
1. sandbox_1887_3M $18.87/QUARTERLY
 || 
...
Indicate user's device platform

Optional value: `android`, `iphone`

If this field is empty or other value, we will use `iphone` as the default value. You can determine whether a device is running IOS or Android by analyzing the User-Agent in HTTP request |
### Response data struct
...
map<string, SubscriptionTradeOrderInfoObject>
...
Length of each deduction cycle. Possible values are:
1. WEEKLY
1. MONTHLY
1. BIMONTHLY
1. QUARTERLY
...
indicate the deduction type. Possible values are:
1. one_time
1. auto_renew
 || 
...
Price to deduct for each cycle || 

currency || 

string || 

Currency of price to deduct for each cycle || 

symbol || 

string || 

Symbol of currency of price to deduct for each cycle |
### Example
