...
Whether the keyboard remains visible when the "Done" button is clicked || 
...
The type of the confirm button in the lower right corner || 
...
Keyboard type (text, or numeric on Client 8.0.57 and above) || 
...
Callback function for the end of interface call || 
...
Current value of the keyboard input box || 
...
Current value of keyboard input |
...
wx.offKeyboardInput(listener) *// The same function object used for listening must be passed in*
`
```
...
wx.offKeyboardHeightChange(listener) *// The same function object used for listening must be passed in*
`
```
...
wx.offKeyboardConfirm(listener)
wx.offKeyboardConfirm(listener) *// The same function object used for listening must be passed in*
`
```
...
wx.offKeyboardComplete(listener)
wx.offKeyboardComplete(listener) *// The same function object used for listening must be passed in*
`
```
...
Does the device use a network proxy? || 

weakNet || 

Boolean || 

Is it in a weak connection environment? |
#### Example
...
Touch array. List of all current touchpoints  || 

changedTouches || 

Array<object> || 

Touch array. The list of touchpoints that triggered this event  || 
...
Vibration intensity type, valid values are: heavy, medium, light  || 
