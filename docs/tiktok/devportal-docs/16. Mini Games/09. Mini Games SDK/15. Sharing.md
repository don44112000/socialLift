...
Forward the link to the displayed image || 
...
Query string. After entering from this forwarded message, you can obtain the query in the launch parameters through the `TTMinis.game.getLaunchOptionsSync()` function or the `TTMinis.game.onShow()` function. || 
