...
Frontend calls:
```
`window.TTMinis.login(*function* (*res*) {
  // ...
...
Currently applied client key || 

Yes || 

`client_secret` || 

Currently applied client secret || 

Yes || 

`code` || 

Frontend ` TTMinis.login() ` Returned authorization code  || 
...
1. The front end initiates a business request 
1. The backend uses the saved `access_token` and `open_id` to call the TikTok server 
1. The backend returns the results to the frontend
...
1. Frontend `TTMinis.init({ clientKey })`
1. Frontend `TTMinis.login()`
1. Frontend obtains `AuthorizationCode`
1. Frontend sends `code` to the backend 
1. Backend calls`/v2/oauth/token/` to exchange for `open_id`, `access_token`, `refresh_token`
1. Backend persistent storage
1. The backend refreshes the token just before it expires 
