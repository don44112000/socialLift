...
- [Enabled in-app purchases](/doc/enable-monetization-features) for your mini game or mini drama
- Configured [silent login](/doc/tiktok-minis-silent-login) for your mini drama
- Configured the payment [webhook callback](/doc/set-up-development-configuration#) address on your app page
...
Your frontend should call the [TikTok Minis ](/doc/minis-sdk-payment#)[SDK payment endpoint](/doc/minis-sdk-payment#) with the `trade_order_id` to initiate the payment process:
```
`TTMinis.game.pay({
...
1. Verify Signature
1. Check if the order has been processed
1. Issue virtual rights and interests to users based on `open_id`.
...
For more information, refer to the [TikTok Minis SDK IAP APIs](/doc/minis-sdk-payment) or [Payment APIs from TikTok Minis](/doc/minis-payment-apis). 
...
<img src="https://sf16-sg.tiktokcdn.com/obj/tiktok-open-platform-sg/ttop_doc_images/0c24e1dadc3cf1bb17c0e01bf22f7781" alt="image" width="2390px" />
...
<img src="https://sf16-sg.tiktokcdn.com/obj/tiktok-open-platform-sg/ttop_doc_images/e061e57702e3bea18368649608970a1e" alt="image" width="1278px" />
...
<img src="https://sf16-sg.tiktokcdn.com/obj/tiktok-open-platform-sg/ttop_doc_images/327c62d3d14a4fa10ef76ebb42eff29f" alt="image" width="1278px" />
...
1. Your client calls `TTMinis.createSubscription` with the `trade_order_id` to open the payment screen.
1. After the user completes payment, TikTok sends a `minis.subscription.create` webhook to your server to confirm activation.

### Reactivate subscription

Note: A subscription can be reactivated ONLY when `is_renew_normal=false`.
<img src="https://sf16-sg.tiktokcdn.com/obj/tiktok-open-platform-sg/ttop_doc_images/db406321e19e8e59c66f0edf9ca8e0a6" alt="image" width="2362px" />
...
1. Your client calls `TTMinis.reactivateSubscription` with the `trade_order_id` to open the payment screen.
1. After the user completes payment, TikTok sends a `minis.subscription.reactivate` webhook to your server to confirm activation.
...
1. First, ensure that the silent login link is available 
1. Confirm that the backend can create `trade_order_id`
1. The front end successfully launched `TTMinis.pay()`
1. Confirm that the backend has received the webhook
1. Confirm that the backend shipment was successful
1. Confirm that the frontend polling status update was successful
