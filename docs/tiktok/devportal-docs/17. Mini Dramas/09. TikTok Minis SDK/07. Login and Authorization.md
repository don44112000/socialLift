...
Authorization scope, default `user.info.basic` || 
...
Whether to include the authorized scope in the return result || 
...
`authResponse.grantedScopes` || 

`string` || 

Actual authorized scope, available when `returnScopes` is enabled || 
