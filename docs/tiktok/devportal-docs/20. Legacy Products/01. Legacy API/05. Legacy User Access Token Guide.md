# Legacy User Access Token Guide

# Manage Legacy User Access Tokens

Legacy user access tokens using our OAuth v1 API are deprecated. [View migration announcement](/bulletin/migration-guidance-oauth-v1).
...
You can learn more about Scopes on our [Scopes Overview page](/doc/scopes-overview).
### Token Security
...
Authorization code from Web/iOS/Android authorization callback. || 
...
The scopes user has agreed to authorize, separated by comma (,). || 
...
The token for refresh the `access_token`. **It is valid for 365 days after initial issuance**. || 

refresh_expires_in || 

int64 || 

Expiration for the `refresh_token` in seconds. |
...
The user's `refresh_token` received from `/oauth/access_token/` endpoint. |
#### Response.Data Struct
...
New token for future calls on behalf of the user. || 

expires_in || 

int64 || 

Expiration for the access token in seconds. || 

refresh_token || 

string || 

The token for refresh an user's `access_token`. 

Note that the returned refresh_token may be different than the one passed in the payload. **Developers must use the newly-returned token should the value is different than the previous one.** || 
...
*POST* `https://open-api.tiktok.com/oauth/revoke/`
#### Request Parameters
...
The TikTok user's unique identifier; obtained through `/oauth/access_token/` the user's unique token. || 

access_token || 

string || 

The token that bears the authorization of the TikTok user, `/oauth/access_token/`. This token requires user authorization. |
#### Response.Data Struct
